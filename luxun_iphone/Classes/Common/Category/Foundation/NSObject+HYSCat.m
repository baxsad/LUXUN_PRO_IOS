...
#import "NSObject+HYSCat.h"
#import <objc/objc.h>
#import <objc/runtime.h>
#import <CommonCrypto/CommonCryptor.h>
@interface ObserverBlock : NSObject
...
@implementation ObserverBlock

- (id)initWithBlock:(void (^)(id x))block{
    self = [super init];
    if (self) {
...
- (void)invoke:(id)x {
    if (_callBack) _callBack(x);
}

@end

@implementation NSObject (HYSCat)
...
+ (void)iur_swizzleClassMethod:(Class)clas originSelector:(SEL)originSel with:(SEL)newSel{
    Method otherMethod  = class_getClassMethod(clas, originSel);
    Method originMethod = class_getClassMethod(clas, newSel);
...
+ (void)iur_swizzleInstanceMethod:(Class)clas originSelector:(SEL)originSel with:(SEL)newSel;{
    Method otherMethod  = class_getInstanceMethod(clas, originSel);
    Method originMethod = class_getInstanceMethod(clas, newSel);
    method_exchangeImplementations(otherMethod, originMethod);
}
...
- (instancetype)hys_addObserver:(nonnull NSObject *)observer forKeyPath:(nonnull NSString *)keyPath
{
    [observer addObserver:observer forKeyPath:keyPath options:NSKeyValueObservingOptionInitial context:nil];
    return self;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    ...
    ObserverBlock * observer = [self observer];
    if (observer) {
        [observer invoke:obj];
    }
}
...
{
    ObserverBlock *target = [[ObserverBlock alloc]
                             initWithBlock:nextBlock];
    [self setObserver:target];
}
...
- (void )setObserver:(ObserverBlock *)observer
{
